...
abstract: >
  The goal of `ggcoverage` is to simplify the process of visualizing genome coverage. It contains functions to load data from BAM, BigWig or BedGraph files, create genome coverage plot, add various annotation to the coverage plot, including GC annotation, gene annotation, transcript annotation, ideogram annotation and peak annotation.
output: 
...
`r Biocpkg("ggcoverage")` is an R package distributed as part of the [CRAN](https://cran.r-project.org/).
...
Once `r Rpackage("ggcoverage")` is installed, it can be loaded by the following command.
...
* **Load the data**: `ggcoverage` can load BAM, BigWig (.bw), BedGraph files from various NGS data, including DNA-seq, RNA-seq, ChIP-seq, ATAC-seq, et al.
...
* **Add annotaions**: `ggcoverage` supports four different annotaions:
  ...
  * **transcription annotion**: Visualize genome coverage across different transcripts
...
## Add annotations
Add **gene**, **ideogram** and **peak** annotaions. To create peak annotaion, we first **get consensus peaks** with [MSPC](https://github.com/Genometric/MSPC), you can also use DEbChIP's `GetConsensusPeak` (`MSPC`'s wrapper) to do this.
