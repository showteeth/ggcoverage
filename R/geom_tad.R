...
geom_tad <- function(matrix, granges, zlim = NULL, color.bias = 1, color.ramp = "YlOrRd",
...
ggplot_add.tad <- function(object, plot, object_name) {
  ...
  tad.plot <- GenomeMatrix::matrixPlotter(
    mat = matrix, granges = granges, plotGR = plot.range.gr,
  ...
  tad.final.plot <- tad.plot +
    labs(y = "ContactMap") +
    theme_tad(x.range = c(plot.range.start, plot.range.end), margin.len = plot.space, show.rect = show.rect)
  # assemble plot
  if (top) {
