...
ggcoverage <- function(data, region = "chr14:21,677,306-21,737,601", gtf.gr = NULL, extend = 2000,
                       gene.name = "HNRNPC", gene.name.type = c("gene_name", "gene_id"),
                       mapping = NULL, color = NULL, facet.key = "Type", facet.order = NULL, facet.color = NULL,
  ...
  coverage.plot <- ggplot() +
    geom_coverage(
      ...
      color = color, facet.key = facet.key, facet.order = facet.color, facet.color = facet.color,
